...
<?php
  // "Next to use" is whichever batch FIFO will draw from next — the
  // oldest one that still has stock left — not simply the first row,
  // since depleted batches (now shown for full history) can sit ahead
  // of it in the oldest-first list.
  $ggNextToUseId = null;
  foreach ($batches as $b) {
      if ($b['remaining_quantity'] > 0) {
          $ggNextToUseId = $b['batch_id'];
          break;
      }
  }

  $ggClassify = static function (array $b, $nextToUseId): string {
      if ($b['batch_id'] === $nextToUseId) {
          return '<span class="badge bg-warning">Next to use</span>';
      }
      if ($b['remaining_quantity'] <= 0) {
          return '<span class="badge bg-secondary">Depleted</span>';
      }
      if ($b['remaining_quantity'] < $b['quantity']) {
          return '<span class="badge bg-info">Partially Used</span>';
      }
...
<?php else: ?>
    ...
    <?php foreach ($batches as $b): ?>
      <div class="card">
        <div class="card-body p-3">
          <div class="d-flex justify-content-between align-items-start mb-1">
            <span class="fw-semibold">Batch #<?= $b['batch_id'] ?></span>
            <?= $ggClassify($b, $ggNextToUseId) ?>
          </div>
  ...
  <div class="table-responsive d-none d-lg-block">
    <table class="table align-middle mb-0 dg-table">
      <thead class="table-light"><tr><th>Batch</th><th>Received</th><th>Remaining / Received Qty</th><th>Status</th><th>Notes</th></tr></thead>
      <tbody>
        <?php foreach ($batches as $b): ?>
          <tr>
            <td class="fw-semibold">#<?= $b['batch_id'] ?></td>
            <td><?= date('M d, Y g:i A', strtotime($b['received_at'])) ?></td>
            <td><?= $b['remaining_quantity'] ?> / <?= $b['quantity'] ?></td>
            <td><?= $ggClassify($b, $ggNextToUseId) ?></td>
